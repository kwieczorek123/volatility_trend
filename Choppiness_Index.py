...
interval_map = {'D1': (1440, '_CI_D1'), 'H1': (60, '_CI_H1')}

# Determine which intervals to process
if interval == 'all':
    intervals_to_process = list(interval_map.keys())
else:
...
for symbol in list_of_symbols:
    for interval in intervals_to_process:
        interval_multiplier, output_suffix = interval_map[interval]

        # Read in the OHLC_data
        df = pd.read_csv(f"Input data/OHLC_data/{symbol}/{symbol}{interval_multiplier}.csv",
                         names=['date', 'time', 'open', 'high', 'low', 'close', 'volume'])

        # Combine date and time, and drop the 'time' column
        ...
        # Function to update trend and in_progress columns
        def update_trend(df, start_date, mid_date, finish_date):
            ...
            if finish_date is not None:
                ...
                df.loc[df.index[df.index.get_loc(finish_date):], 'in_progress'] = 0
            return df


        # Initialize trend, in_progress columns with all zeros
        df['trend'] = 0
        df['in_progress'] = 0
                ...
                if df['CI'].iloc[i] < lower_band and mid_date is None:
                    ...
                    for j in range(df.index.get_loc(mid_date) + 1, len(df)):
                        if df['CI'].iloc[j] > lower_band:
                            ...
                            df.loc[df.index[j], 'in_progress'] = 0

                    # Set the trend and in_progress to 0 from finish_date onwards
                    if finish_date is not None:
                                            ...
                                            if df['CI'].iloc[k] < lower_band:
                                                ...
                                                for l in range(df.index.get_loc(new2_mid_date) + 1, len(df)):
                                                    if df['CI'].iloc[l] > lower_band:
                                                        ...
                                                        df.loc[df.index[l], 'in_progress'] = 0
                                                break
                                        break
                                    else:
                                        df.loc[df.index[l], 'trend'] = 1
        ...
        for i in range(len(df)):
            ...
            if df['trend'].iloc[i] == 1 and df['in_progress'].iloc[i] == 0:
        ...
        df.to_csv(f'results/trend_volatility_results/processed_CI_data/{symbol}{output_suffix}.csv')
