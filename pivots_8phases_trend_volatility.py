...
file_names_pivots_long = ['results/trend_volatility_results/processed_CI_data/EURUSD_CI_D1.csv',
                          'results/trend_volatility_results/processed_CI_data/GBPUSD_CI_D1.csv',
...
# Function to add typical_spread, and execution spread columns to the input DataFrame
def add_spread_columns(df, spread_df, execution_df, symbol):
    ...
    df = df.merge(execution_df.loc[execution_df['symbol_name'] == symbol, ['date', 'vol_avg_spread', 'volume']],
                  on='date', how='left')
    df = df.rename(columns={'vol_avg_spread': 'weighted_avg_execution_spread_$', 'volume': 'volume_spreads_df'})

    return df


# Function to add profit, volume and PnL_per_lot columns to the input DataFrame
def add_pnl_per_lot_column(df, pnl_df, symbol):
    df = df.merge(pnl_df.loc[pnl_df['symbol_name'] == symbol, ['day', 'PnL_per_lot', 'profit', 'volume']],
                  left_on='date', right_on='day', how='left')
    df = df.rename(columns={'profit': 'total_profit', 'volume': 'total_volume'})
...
def process_csv(file_name, start_date, end_date, spread_df, execution_df, pnl_df_dict):
    ...
    output_dir = "results/trend_volatility_results/processed_8phases_data"
...
for file_name in file_names_pivots_long:
    ...
    for symbol in ['EURUSD', 'GBPUSD', 'USDJPY', 'XAUUSD']:
        ...
        pivot = pivot.reindex(
            columns=['count_of_occurrences', 'percentage_of_occurrences', 'typical_spread_in_points',
                     'volume_weighted_avg_spread_in_USD', 'PnL_per_lot', 'total_profit', 'pct_total_profit',
                     'total_volume', 'pct_total_volume', 'pct_impact_on_PnL_exec_spread'])
        ...
        output_dir = f"results/trend_volatility_results/processed_8phases_data/pivots"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        wb.save(f"{output_dir}/pivots_8phases_{interval}.xlsx")


# Function to create pivot tables using the processed OHLC_data
def create_pivot_tables_and_charts_actionable(processed_files, interval):
    ...
    for symbol in ['EURUSD', 'GBPUSD', 'USDJPY', 'XAUUSD']:
        ...
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        wb.save(f"{output_dir}/pivots_8phases_actionable_{interval}.xlsx")
...
def create_pivot_tables_and_charts_long(processed_files):
    ...
    wb.save("results/trend_volatility_results/processed_8phases_data/pivots/pivots_8phases_long.xlsx")
...
def process_all_intervals(interval_list):
    for interval in interval_list:
        ...
        # Update the execution spread and pnl files for the current interval
        execution_spread_file = f'Input data/trend_volatility_input_data/execution_spread_{interval}.csv'
        pnl_file = f'Input data/trend_volatility_input_data/PnL_data_{interval}.csv'
        ...
        pnl_df = pd.read_csv(pnl_file, parse_dates=['day'])
        pnl_df_dict = {symbol: pnl_df[pnl_df['symbol_name'] == symbol] for symbol in
                       ['EURUSD', 'GBPUSD', 'USDJPY', 'XAUUSD']}
        ...
        for file_name in file_names:
            ...
            output_file_path = os.path.join("results/trend_volatility_results/processed_8phases_data",
...
interval = 'all'  # Change this to 'D1', 'H1', or 'all'
if interval == 'all':
    process_all_intervals(['D1', 'H1'])
else:
...
create_pivot_tables_and_charts_long([
    "results/trend_volatility_results/processed_4phases_data/processed_EURUSD_vol_trend_long_D1.csv",
    "results/trend_volatility_results/processed_4phases_data/processed_GBPUSD_vol_trend_long_D1.csv",
    "results/trend_volatility_results/processed_4phases_data/processed_USDJPY_vol_trend_long_D1.csv",
    "results/trend_volatility_results/processed_4phases_data/processed_XAUUSD_vol_trend_long_D1.csv"
])
